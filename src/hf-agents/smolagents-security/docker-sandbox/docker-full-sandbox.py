    ...
    def create_container(self):
        try:
            image, build_logs = self.client.images.build(
                path="/home/markus/projects/agent-basics/src/hf-agents/smolagents-security/docker-sandbox/",
                tag="agent-sandbox",
                rm=True,
                forcerm=True,
                buildargs={},
...
agent = CodeAgent(
    model=InferenceClientModel("meta-llama/Llama-4-Scout-17B-16E-Instruct", token=os.getenv("HF_TOKEN"), provider="novita"),
    tools=[]
)

# Run the agent
response = agent.run("What's the 10th Fibonacci number?")
print(response)
"""
